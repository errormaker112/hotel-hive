...
def createOwnerView(request):
    ...
    if owner_serializer.is_valid():
        ...
        # is_approved is False by default — admin must approve
        return Response(
...
def getManagerList(request):
    ...
    managerdata = [
        {
            'id': manager.id,
            'name': f'{manager.user.first_name} {manager.user.last_name}',
...
def deleteManager(request):
    ...
    try:
        ...
        return Response({'detail': 'Manager does not exist.'}, status=status.HTTP_404_NOT_FOUND)
